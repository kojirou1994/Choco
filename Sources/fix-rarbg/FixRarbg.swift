          ...
          .forEachEntries { entry, _ in
            ...
            do {
              ...
              if delete {
                ...
                if deleteUnused {
                  subs.forEach { sub in
                    try? SystemFileManager.remove(sub)
                  }
                } else {
                  usedSubInfos.forEach { sub in
                    try? SystemFileManager.remove(sub.path)
                  }
                }
                try SystemFileManager.remove(filePath)
              }
            } catch {
              print("ERROR: \(error)")
            }
