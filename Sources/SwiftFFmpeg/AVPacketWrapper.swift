...
//internal typealias AVPacket = CFFmpeg.AVPacket
...
public final class AVPacketWrapper {
    internal let packetPtr: UnsafeMutablePointer<AVPacket>
    internal var packet: AVPacket { return packetPtr.pointee }

    internal init(packetPtr: UnsafeMutablePointer<AVPacket>) {
    ...
    public var buf: AVBufferWrapper? {
        get {
            if let bufPtr = packet.buf {
                return AVBufferWrapper(bufPtr: bufPtr)
            }
            return nil
        }
        set { packetPtr.pointee.buf = newValue?._avBuffer }
    }
    ...
    ///
    /// - Throws: AVerror
    public func ref(dst: AVPacketWrapper) throws {
    ...
    public func clone() -> AVPacketWrapper? {
        if let ptr = av_packet_clone(packetPtr) {
            return AVPacketWrapper(packetPtr: ptr)
        }
        return nil
    }
    ...
    deinit {
        var ptr: UnsafeMutablePointer<AVPacket>? = packetPtr
        av_packet_free(&ptr)
    }
}
